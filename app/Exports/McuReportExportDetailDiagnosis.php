...
class McuReportExportDetailDiagnosis implements FromQuery, WithHeadings, ShouldAutoSize, WithMapping
{
    protected $idPasien;
    protected $nama;
    protected $nip;
	...
	public function query(){
		...
		// $m = $m->join(DB::raw("(SELECT d.mcu_id, c.code, c.name from diagnoses d
				// left join recommendations a on d.recommendation_id = a.id
				// left join icd10s c on c.id = a.icd10_id
				// where c.code = '$this->code')
                // m"), 
				// function($join)
				// {
				   // $join->on('mcu.id', '=', 'm.mcu_id');
				// });
