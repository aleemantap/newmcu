...
class McuReportExportDetailDiagnosisDkk implements FromQuery, WithHeadings, ShouldAutoSize, WithMapping
{
    protected $idPasien;
    protected $nama;
    protected $nip;
	...
	public function query(){
		...
		$m->whereHas('diagnosis.recommendation', function($q) use ($cd,$id_wh){
			 
			 if($id_wh != '')
			 {
			 	 $q->where('work_health_id', $id_wh);
			 }

			 $q->whereHas('icd10',function($v) use ($cd){
			 	$v->where('code', $cd);
			 });
